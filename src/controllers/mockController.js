...
export const runAsyncMockBuilder = async (userId, monthlyIncome, jobType, linkedAccounts = []) => {
  try {
    ...
    // panggil generator
    const rawTransactions = createMockTransactions(jobType, monthlyIncome, linkedAccounts, 'full')
    ...
    console.log(
      `[BACKGROUND JOB] Meminta AI di Railway memproses ${uniqueDescriptions.length} teks unik...`
    )
    ...
    // aggregation 12 bulan
    const now = new Date()

    for (let i = 0; i < 12; i++) {
      const d = new Date(now.getFullYear(), now.getMonth() - i, 1)

      await calculateMonthlyAggregation(userId, d.getFullYear(), d.getMonth() + 1)
    }
    ...
    await prisma.user.update({
      ...
      data: {
        ...
        currentSavings: Math.max(0, (monthlyIncome || 0) - avgMonthlyExpense),
      },
    })

    console.log(
      `[BACKGROUND JOB] Sistem siap! Seluruh profil pengguna ${userId} telah ter-generate.\n`
    )
  } catch (error) {
