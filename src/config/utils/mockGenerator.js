...
export const createMockTransactions = (jobType = "permanent", statedIncome = 5000000, linkedAccounts = []) => {
  ...
  const userBanks = linkedAccounts.filter(acc => acc.type === 'bank').map(acc => acc.provider.toLowerCase());
  const userEwallets = linkedAccounts.filter(acc => acc.type === 'ewallet').map(acc => acc.provider.toLowerCase());
  ...
  const primaryBank = userBanks.length > 0 ? userBanks[0] : fallbackBanks[Math.floor(Math.random() * fallbackBanks.length)];
  
  // Tentukan Bank Sekunder (Gunakan bank kedua jika user pilih > 1 bank. Jika tidak, ambil fallback selain bank utama)
  let secondaryBank = userBanks.length > 1 ? userBanks[1] : fallbackBanks.find(b => b !== primaryBank);
  ...
  const primaryEwallet = userEwallets.length > 0 ? userEwallets[0] : fallbackEwallets[Math.floor(Math.random() * fallbackEwallets.length)];
  ...
  while (currentMonth <= endDate) {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (Math.random() < myFoodHabit) { 
         const isStudent = (jobType === "not_working" || jobType === "student" || jobType === "Mahasiswa");
         const foodDesc = isStudent ? (Math.random() > 0.5 ? "GOFOOD WARMINDO" : "KOPI KENANGAN") : (Math.random() > 0.5 ? "GOFOOD AYAM GEPREK" : "STARBUCKS");
         const foodPrice = isStudent ? (Math.floor(Math.random() * 20000) + 15000) : (Math.floor(Math.random() * 50000) + 35000);

         generatedTransactions.push({
            dateTime: new Date(year, month, day, 12 + Math.floor(Math.random()*8), 0, 0).toISOString(),
